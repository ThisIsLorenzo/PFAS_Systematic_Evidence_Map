...
Setting global code chunk parameters and loading packages.
...
A list of included studies during the supplementary search of all types of reviews for supplementary bibliometric analyses is included in **main_all_types_reviews.csv**
...
Using the same code to create supplementary bibliometric adata on all types of reviews.
```{r, eval=FALSE}
bib_data_all <- convert2df(here("data","scopus_all_types.csv"), dbsource = "scopus", format = "csv")
dim(bib_data_all) #517 rows 52 columns

bib_data_all <- metaTagExtraction(bib_data_all, Field = "AU1_CO",
                          sep = ";")
...
The following chunk creates a search string based on documents DOIs. We used the search string to find the documents on the Scopus database and perform the main bibliometric analysis.
...
The following chunk creates a search string based on documents DOIs. Documents are all types of review. We used the search string to find the documents on the Scopus database and perform the supplementary bibliometric analysis.
...
Summary tables of all included systematic reviews in the map
...
Summary tables of all types of reviews included in the supplementary bibliometric analysis.
...
Some MeSH headings also have qualifiers (sometimes called subheadings) - these are shown after a forward-slash and are used to "describe the specific aspects of the MeSH heading that are pertinent to the article" (<https://www.nlm.nih.gov/bsd/indexing/training/SUB_010.html>).
MeSH terms headings and qualifiers could be analysed separately.
...
## COMBINED plot
```{r, eval=FALSE}
# png("Figs/Fig.3.png", width = 24, height = 16, units = "cm", res = 300)
...
##COMBINED plot
```{r, eval=FALSE}
# png("Figs/Fig4.png", width = 20, height = 16, units = "cm", res = 300)
...
```{r}
tt2.1/tt2/tt3/tt4
ggsave(here("Figs", "Fig.2.png"),
       width = 18,
       height = 28,
       units = "cm")

# png("Figs/Fig.2.png", width = 18, height = 25, units = "cm", res = 300)
# tt2.1/tt3/tt2/tt4
# dev.off()
```
...
# t_tx <- t_tx %>%
#   mutate(Class_scientific_name = if_else(
#     Class_scientific_name %in% c('Mammal', 'Mammalia'), 'Mammalia',
#     Class_scientific_name %in% c('Brachiopoda', 'Brachiopods'), 'Brachiopoda',
...
###Broad taxa in wildlife research
...
sp2.1-sp1.1
# ggsave(here("Figs/Suppl_Figs", "SFig.6.1.png"),
#        width = 8,
#        height = 9)
...
ggplot(species_lab_wildlife, aes(x = Lab_domestic_wildlife_mixed, 
  ...
  geom_col(aes(color = "black",
          ...
          width = 0.8,
          size = 0.2) +
  labs(title = expression(bold("Laboratory VS wildife"))) +
  theme_light() +
  coord_flip()  +
  scale_fill_brewer() +
  scale_fill_manual(values = c("#999999")) +
...
```{r, eval=FALSE}
png("Figs/Fig.6.png", width = 18, height = 22, units = "cm", res = 300)
...
# pdf(file="./Figs/Suppl_Figs/SFig.14.pdf",
#     width = 6, 
#     height = 5, 
#     pointsize = 10,
#     family = "Helvetica",
...
```{r, eval = FALSE}
pdf(here("Figs", "Fig.7.pdf"), width = 8, height = 12)
...
```
#-------------------------------------------------------------------
#Bibliometric - all types of reviews
...
Saving panel as a PDF
```{r, eval = FALSE}
pdf(here("Figs/Suppl_Figs", "SFig.19.pdf"), width = 8, height = 12)
